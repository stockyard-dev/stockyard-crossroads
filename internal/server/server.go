...
func New(db *store.DB, limits Limits, dataDir string) *Server {
	...
	s.mux.HandleFunc("GET /api/links", s.list)
	s.mux.HandleFunc("POST /api/links", s.create)
	s.mux.HandleFunc("GET /api/links/{id}", s.get)
	s.mux.HandleFunc("DELETE /api/links/{id}", s.del)
	...
	s.mux.HandleFunc("GET /api/tier", func(w http.ResponseWriter, r *http.Request) {
		wj(w, 200, map[string]any{"tier": s.limits.Tier, "upgrade_url": "https://stockyard.dev/crossroads/"})
	})
...
func (s *Server) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if path != "/" && !strings.HasPrefix(path, "/api/") && !strings.HasPrefix(path, "/ui") {
		...
		if slug != "" && !strings.Contains(slug, "/") {
			l := s.db.GetBySlug(slug)
			if l != nil {
				s.db.RecordClick(slug, r.RemoteAddr, r.UserAgent(), r.Referer())
...
func (s *Server) list(w http.ResponseWriter, r *http.Request) {
	wj(w, 200, map[string]any{"links": oe(s.db.List())})
}
func (s *Server) create(w http.ResponseWriter, r *http.Request) {
	...
	s.db.Create(&l)
	wj(w, 201, s.db.GetByID(l.ID))
}
func (s *Server) get(w http.ResponseWriter, r *http.Request) {
	l := s.db.GetByID(r.PathValue("id"))
	if l == nil {
		we(w, 404, "not found")
		return
	}
...
func (s *Server) clicks(w http.ResponseWriter, r *http.Request) {
	wj(w, 200, map[string]any{"clicks": oe(s.db.ClickHistory(r.PathValue("id"), 50))})
}
func (s *Server) stats(w http.ResponseWriter, r *http.Request) { wj(w, 200, s.db.Stats()) }
func (s *Server) health(w http.ResponseWriter, r *http.Request) {
	st := s.db.Stats()
	wj(w, 200, map[string]any{"status": "ok", "service": "crossroads", "links": st.Links, "clicks": st.TotalClicks})
}
func oe[T any](s []T) []T {
	if s == nil {
		return []T{}
...
func (s *Server) loadPersonalConfig() {
	...
	if err := json.Unmarshal(data, &cfg); err != nil {
		log.Printf("%s: warning: could not parse config.json: %v", "crossroads", err)
		return
	}
	s.pCfg = cfg
	log.Printf("%s: loaded personalization from %s", "crossroads", path)
}
