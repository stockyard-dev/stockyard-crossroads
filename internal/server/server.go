package server
import("encoding/json";"net/http";"github.com/stockyard-dev/stockyard-crossroads/internal/store")
...
func(s *Server)routes(){
    ...
    s.mux.HandleFunc("GET /p/{slug}",s.handleGetPublic)
    s.mux.HandleFunc("GET /api/profiles/{id}/links",s.handleListLinks)
    s.mux.HandleFunc("POST /api/profiles/{id}/links",s.handleCreateLink)
    s.mux.HandleFunc("POST /api/links/{id}/toggle",s.handleToggleLink)
    s.mux.HandleFunc("DELETE /api/links/{id}",s.handleDeleteLink)
    s.mux.HandleFunc("POST /api/links/{id}/click",s.handleClick)
    s.mux.HandleFunc("GET /",s.handleUI)
}
func(s *Server)handleHealth(w http.ResponseWriter,r *http.Request){writeJSON(w,200,map[string]string{"status":"ok","service":"stockyard-crossroads"})}
